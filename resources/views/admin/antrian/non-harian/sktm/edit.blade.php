              ...
              <div class="card-body">
                <table class="table">
                    <tbody>
                      ...
                      <tr>
                        <td><label>Sekolah/Universitas</label></td>
                        <td>{{ $sktm->sekolah }}</td>
                      </tr>
                      <tr>
                        <td><label>Kelas/Tingkat</label></td>
                        <td>{{ $sktm->kelas }}</td>
                      </tr>
                    </tbody>
                  </table>
            ...
            <div class="card px-4">
              <div class="card-body pb-3">
                 ...
                 <div class="row mb-3">
                  ...
                  <div class="col-md-9">
                    <a href="#exampleModal" data-toggle="modal" data-target="#exampleModal">{{ $sktm->user->nama }}</a> <br>
                    <b>{{ $sktm->jenis }}</b> <br>
                    <form action="{{ route('antrian-sktm.update', $sktm->id) }}" method="post">
                      @method('patch')
                      @csrf
                      <select class="form-control" name="keterangan" id="keterangan">
                        <option value="{{ $sktm->keterangan }}">dari pengaju : {{ $sktm->keterangan }}</option>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="{{ route('sktm.tolak', $sktm->id) }}" method="post">
          @csrf
          <div class="mb-3">
      ...
      <div class="modal-body">
        <div class="row mb-3">
         ...
         <div class="col-md-9 pl-1">
           <a href="{{ route('user.show', $sktm->user->id) }}">{{ $sktm->user->nama }}</a> <br>
           <b>{{ $sktm->user->nik }}</b> <br>
           <span>{{ $sktm->user->ttl }}</span> <br>
           <span>{{ $sktm->user->agama }}</span> <br>
           <span>{{ $sktm->user->jk }}</span> <br>
           <span>{{ $sktm->user->status }}</span> <br>
           <span>{{ $sktm->user->pekerjaan }}</span> <br>
           <span>{{ $sktm->user->rt }}/{{ $sktm->user->rw }}</span> <br>
           <span>{{ $sktm->user->alamat }}</span> <br>
         </div>
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Tutup</button>
        <a href="{{ route('user.show', $sktm->user->id) }}" class="btn btn-primary">Lihat lebih banyak</a>
