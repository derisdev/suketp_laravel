            ...
            <div class="card px-3">
              ...
              <div class="card-body">
                <table class="table">
                    <tbody>
                      <tr>
                        <td><label>Klarifikasi yang bersangkutan</label></td>
                        <td>{{ $skck->klarifikasi }}</td>
                      </tr>
                      <tr>
                ...
                <table class="table">
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>Rt/Rw : {{ $skck->user->rt }}/{{ $skck->user->rw }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            <div class="card px-4">
              <div class="card-body pb-3">
                 ...
                 <div class="row mb-3">
                  ...
                  <div class="col-md-9">
                    <a href="#exampleModal" data-toggle="modal" data-target="#exampleModal">{{ $skck->user->nama }}</a> <br>
                    <b>{{ $skck->jenis }}</b> <br>
                    <form action="{{ route('antrian-skck.update', $skck->id) }}" method="post">
                      @method('patch')
                      @csrf
                      <select class="form-control" name="keterangan" id="keterangan">
                        <option value="{{ $skck->keterangan }}">dari pengaju : {{ $skck->keterangan }}</option>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="{{ route('skck.tolak', $skck->id) }}" method="post">
          @csrf
          <div class="mb-3">
      ...
      <div class="modal-body">
        <div class="row mb-3">
         ...
         <div class="col-md-9 pl-1">
           <a href="{{ route('user.show', $skck->user->id) }}">{{ $skck->user->nama }}</a> <br>
           <b>{{ $skck->user->nik }}</b> <br>
           <span>{{ $skck->user->ttl }}</span> <br>
           <span>{{ $skck->user->agama }}</span> <br>
           <span>{{ $skck->user->jk }}</span> <br>
           <span>{{ $skck->user->status }}</span> <br>
           <span>{{ $skck->user->pekerjaan }}</span> <br>
           <span>{{ $skck->user->rt }}/{{ $skck->user->rw }}</span> <br>
           <span>{{ $skck->user->alamat }}</span> <br>
         </div>
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Tutup</button>
        <a href="{{ route('user.show', $skck->user->id) }}" class="btn btn-primary">Lihat lebih banyak</a>
